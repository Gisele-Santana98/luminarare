const dados = [
  { numero: "1 em 17", label: "pessoas no mundo tem uma doença rara" },
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="flex-1 grid grid-cols-2 gap-6">
          {dados.map((item) => (
            ...
            >
              <span className="text-3xl font-bold text-[#4338CA]">{item.numero}</span>
              <span className="text-[#475569] text-sm leading-snug">{item.label}</span>
            </div>
          ))}
        </div>
